...
export function parseHoroscopeQueryIntent(text) {
  ...
  // "(моя\s+)?" — без этого "моя натальная карта" не матчилось вообще
  // (якорь ^ требовал, чтобы "натальная" было первым словом буквально) и
  // проваливалось в обычный чат, где LLM просто по памяти о своих же
  // умениях придумывала ответ вместо честной проверки сохранённых данных.
  if (/^(моя\s+)?натальн[а-яёА-ЯЁ]*\s+карт[а-яёА-ЯЁ]*/i.test(t)) {
    return { type: "chart", chartLabel: "натальная карта" };
  }
  if (new RegExp(`^(мои|мой)\\s+характер${NOT_CYRILLIC_LETTER}`, "i").test(t)) {
    // "мои/мой" тут ОБЯЗАТЕЛЕН (раньше был опционален) — по той же причине,
    // что и с "мои аспекты" ниже: голое "характер X" в начале сообщения —
    // частая формулировка вопроса про персонажа/человека вообще ("характер
    // Железного человека", "характер у него сложный"), а не про личный
    // разбор натальной карты. Без обязательного "мой" такие вопросы ложно
    // улетали в астрологию.
    // Расчёт и данные для LLM идентичны "натальной карте" (это один и тот
    // же buildChartContext) — chartLabel тут только для заголовка в чате,
    // чтобы он не говорил "Натальная карта" в ответ на "мой характер".
    return { type: "chart", chartLabel: "разбор характера" };
  }
  // "мои" тут так же обязателен, как теперь и у "характер" выше — голое
  // слово "аспекты" слишком частое в обычной речи (аспекты проекта, аспекты
