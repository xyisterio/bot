...
- Перед сообщением иногда может стоять метка вида "[фото от Имя: описание]" — значит в чате есть картинка: либо её только что прислал тебе сам собеседник, либо это фото другого участника чата, а собеседник ответил на него вопросом или репликой в твою сторону (тогда имя в метке будет отличаться от имени в "Имя: текст" сразу после неё — учитывай это: ты комментируешь чужое фото по просьбе того, кто сейчас с тобой говорит). Описание в метке — это то, что реально видно на фото (распознано автоматически заранее), не выдумывай подробностей сверх него. Реагируй на фото живо и по-своему, с реальной оценкой или мнением — не просто пересказывай описание своими словами. Саму метку никогда не комментируй и не упоминай вслух.
...
bot.on("message:photo", async (ctx) => {
  ...
  // Пересланное фото (мем/новость из канала) — не логируем в ChatLog, как
  // и пересланный текст (см. isForwardedMessage), но если оно всё же явно
  // адресовано боту (переслали и подписали "Женя, глянь") — всё равно
  // стоит ответить, поэтому фильтр только на логирование, не на toBot-ветку.
...
bot.on("message:text", async (ctx) => {
  ...
  if (isGroup) {
    ...
    // Реплай на чужое фото (не на сообщение самого бота) с вопросом/репликой
    // к боту — например "Женя как тебе?" ответом на фото еды, которое
    // прислал Александр. Достаём запомненное описание именно этого фото
    // (см. rememberPhotoCaption в bot.on("message:photo")) и добавляем тег
    // "[фото от Имя: описание]", который разобран в SYSTEM_PROMPT — так
    // модель понимает, что комментирует чужую картинку по чужой просьбе,
    // а не выдумывает, что там нарисовано.
    const repliedTo = ctx.message.reply_to_message;
    if (repliedTo?.photo && repliedTo.from?.id !== ctx.me.id) {
      const repliedPhoto = getPhotoCaption(chatId, repliedTo.message_id);
      if (repliedPhoto) {
        userText = `[фото от ${repliedPhoto.name}: ${repliedPhoto.caption}] ${userText}`;
      }
    }
  }
