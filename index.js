...
async function loadKrokodilDictionary() {
  try {
    ...
    // Размеры уровней специально держим примерно равными (по ~2-2.5к слов) —
    // раньше "сложных" было в разы больше, чем "лёгких"/"средних":
    //  - 0..2000: топовая бытовая лексика ("день", "человек", "деньги");
    //  - 2000..4000: узнаваемые, но не самые частые слова ("миллиардер",
    //    "интерфейс", "казино") — то, что раньше ошибочно уезжало в "hard";
    //  - 4000..6500: собственно редкая/книжная/специальная лексика
    //    ("инквизитор", "холокост", "скарлатина", "тетраэдр", "шариат") —
    //    то, что просили под "сложные". Дальше по списку (после ~9000-14000)
    //    в исходных данных начинаются диалектизмы и полумусорные словоформы
    //    (типа "гуммигут", "кинкажу"), непригодные для игры — не берём.
    krokodilDictionary = {
      easy: filtered.slice(0, 2000),
      medium: filtered.slice(2000, 4000),
      hard: filtered.slice(4000, 6500),
    };
