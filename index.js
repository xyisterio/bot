...
bot.on("message:text", async (ctx) => {
  ...
  if (replyMsg && replyMsg.from?.is_bot) {
    ...
    // по тексту реплая), ТОЛЬКО если это сообщение реально помечено как
    // часть Deezer-потока (см. markDeezerFlowMessage/isDeezerFlowMessage) —
    // раньше это определялось нечётким regex'ом по тексту сообщения
    // (наличие слов "Загружаю"/"найден"/"Deezer"/🎵/🎚), из-за чего обычный
    // ответ LLM (например, рассказ о своих возможностях), случайно
    // упомянувший одно из этих слов, заставлял бота принимать ЛЮБОЙ
    // следующий реплай на него (даже "какая погода в Киеве?") за поиск
