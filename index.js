...
async function loadKrokodilDictionary() {
  try {
    ...
    //  - 0..600: топовая бытовая лексика ("день", "человек", "деньги");
    //  - 600..4000: узнаваемые, но не самые частые слова ("миллиардер",
    //    "интерфейс", "казино") — то, что раньше ошибочно уезжало в "hard";
    //  - 4000..14000: собственно редкая/книжная/специальная лексика
    //    ("трепанация", "инквизитор", "холокост", "скарлатина", "тетраэдр",
    //    "маккартизм", "шариат") — то, что просили под "сложные";
    //  - дальше 14000 в исходных данных всё быстрее скатывается в
    //    диалектизмы, узкоспециальные термины и полумусорные словоформы
    //    (типа "гуммигут", "кинкажу"), непригодные для игры — не берём.
    krokodilDictionary = {
      easy: filtered.slice(0, 600),
      medium: filtered.slice(600, 4000),
      hard: filtered.slice(4000, 14000),
    };
