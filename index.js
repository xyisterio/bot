...
async function handleDeezerQuery(ctx, query) {
  try {
    ...
    // Проверяем кэш file_id — если трек уже отправлялся, используем Telegram file_id (без трафика)
    const cacheKey = `deezer_fid:${bestTrack.id}:${format}`;
    const cachedFileId = await redis.get(cacheKey);

    let sentMsg;
    if (cachedFileId) {
      // Отправляем по file_id — Render не тратит трафик
      ...
      // Первый раз: качаем с HuggingFace через Render (стрим, не буфер)
      const streamUrl = getDeezerStreamUrl(bestTrack.id, format);
      const res = await fetch(streamUrl);
      if (!res.ok) throw new Error(`HuggingFace stream error: ${res.status}`);

      const fileName = `${bestTrack.artist?.name || "Track"} - ${bestTrack.title}.${format === "FLAC" ? "flac" : "mp3"}`;
